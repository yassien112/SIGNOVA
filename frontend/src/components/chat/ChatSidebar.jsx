...
function UserAvatar({ name, size = 40 }) {
  ...
  return (
    <div style={{
      ...
      color: 'white', fontWeight: 700, fontSize: size * .35, flexShrink: 0,
    }}>{name?.charAt(0)?.toUpperCase() ?? '?'}</div>
  );
}

export default function ChatSidebar({
  chats, activeChat, onSelect, onNewChat, loadingChats, myId, unreadCounts = {},
}) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (!showNew) return;
    const handle = (e) => { if (panelRef.current && !panelRef.current.contains(e.target)) setShowNew(false); };
  ...
  const handleOpenNew = () => { setShowNew((v) => !v); setSelectedUser(null); setUserQuery(''); setCreateErr(''); };
  const handleStartChat = async () => {
    if (!selectedUser) return;
    setCreating(true); setCreateErr('');
    try { await onNewChat(selectedUser.email); setShowNew(false); setSelectedUser(null); setUserQuery(''); }
    ...
    <aside className="chat-sidebar">
      <div className="chat-sidebar-header">
        <div className="chat-sidebar-top">
          ...
          <button className={`chat-add-btn${showNew ? ' active' : ''}`} onClick={handleOpenNew}>
        ...
        {showNew && (
          <div className="new-chat-panel" ref={panelRef}>
            <p className="new-chat-panel-title">New conversation</p>
            <div className="new-chat-search">
              <Search size={13} style={{ color: '#6b7280', flexShrink: 0 }} />
              <input type="text" placeholder="Search name or email…" value={userQuery}
                onChange={(e) => { setUserQuery(e.target.value); setSelectedUser(null); }} autoFocus />
              {userQuery && <button onClick={() => { setUserQuery(''); setSelectedUser(null); }}
                style={{ background:'none',border:'none',color:'#6b7280',cursor:'pointer',display:'flex' }}><X size={13} /></button>}
            </div>
            {selectedUser && (
                    ...
                    <button key={u.id}
                      ...
                      <div style={{ position:'relative',flexShrink:0 }}>
                        ...
                        <span style={{ position:'absolute',bottom:0,right:0,width:9,height:9,borderRadius:'50%',
                          border:'2px solid #1a2233',background:u.isOnline?'#22c55e':'#4b5563' }} />
                      </div>
                      <div style={{ flex:1,minWidth:0,textAlign:'left' }}>
                        <p style={{ color:'white',fontSize:'.8rem',fontWeight:600,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{u.name}</p>
                        <p style={{ color:'#6b7280',fontSize:'.7rem',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{u.email}</p>
                      </div>
                      {selectedUser?.id === u.id && <Check size={14} style={{ color:'#34d399',flexShrink:0 }} />}
                    </button>
                  ))}
            </div>
            ...
            <button className="btn-primary"
              ...
              disabled={!selectedUser || creating} onClick={handleStartChat}>
      ...
      <div className="chat-list">
        {loadingChats ? (
          <div className="chat-list-loading">{[1,2,3].map((i) => <div key={i} className="chat-skeleton" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="chat-list-empty"><User size={28} style={{ opacity:.3 }} /><p>{search ? 'No results' : t('noChats')}</p></div>
        ) : (
          filtered.map((chat) => {
              ...
              <button key={chat.id} onClick={() => onSelect(chat)}
                ...
                <div style={{ position:'relative',flexShrink:0 }}>
                  ...
                  <span style={{ position:'absolute',bottom:1,right:1,width:10,height:10,
                    borderRadius:'50%',border:'2px solid #111827',background:isOnline?'#22c55e':'#4b5563' }} />
                </div>
