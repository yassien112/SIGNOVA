      ...
      <div className="chat-sidebar-header">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="chat-new-form">
            ...
            <button type="submit" disabled={creating} className="btn-primary" style={{fontSize:'.75rem',padding:'.375rem .75rem'}}>
            ...
            return (
              <button key={chat.id} onClick={() => onSelect(chat)}
                ...
                <div className="chat-item-avatar">
                  ...
                  <span className="status-online" style={{position:'absolute',bottom:0,right:0}} />
                </div>
                <div className="chat-item-info">
                  <p className="chat-item-name">{label}</p>
                  <p className="chat-item-last">
                    {lastMsg ? (lastMsg.kind==='sign' ? `[${t('signMessage')}]` : lastMsg.text) : t('noMessages')}
                  </p>
                </div>
              </button>
            );
