  ...
  chatNs.on('connection', (socket) => {
      ...
      if (userId) {
        try {
          const chat = await prisma.chat.findFirst({
            where: { id: chatId, OR: [{ isGlobal: true }, { participants: { some: { id: userId } } }] },
            select: { id: true },
          });
          if (!chat) { socket.emit('error', { message: 'Chat not found or access denied' }); return; }
        } catch (err) { console.error('join_chat DB error:', err); }
      }
    ...
    socket.on('send_message', async (data) => {
      try {
        ...
        const kind     = MESSAGE_KINDS.has(data?.kind) ? data.kind : 'text';
        if (!senderId || !chatId || !text) return;

        const chat = await prisma.chat.findFirst({
          where: { id: chatId, OR: [{ isGlobal: true }, { participants: { some: { id: senderId } } }] },
        ...
        const payload = {
          ...
          reactions:    [],
          signs:        savedMessage.metadata?.signs || [],
          segments:     savedMessage.metadata?.segments || [],
          matchedWords: savedMessage.metadata?.matchedWords || [],
          missingWords: savedMessage.metadata?.missingWords || [],
        };
        ...
        if (room) {
          for (const socketId of room) {
            ...
            if (s && s.data.userId && s.data.userId !== senderId) { hasOtherOnline = true; break; }
          }
        }
        if (hasOtherOnline) {
          await prisma.message.update({ where: { id: savedMessage.id }, data: { status: 'delivered' } });
          chatNs.to(normalizeChatRoom(chatId)).emit('message_status', { messageId: savedMessage.id, status: 'delivered' });
        }

        // Push notification to offline participants
        const sender = savedMessage.sender;
        const offlineParticipants = chat.participants.filter((p) => p.id !== senderId);
        for (const participant of offlineParticipants) {
          const userRoom = io.of('/chat').adapter.rooms?.get(`user:${participant.id}`);
          const isInChatRoom = chatNs.adapter.rooms?.get(normalizeChatRoom(chatId));
          if (!isInChatRoom || !userRoom) {
            await notifyNewMessage(prisma, io, {
    ...
    socket.on('typing', ({ chatId, userId, userName }) => {
      ...
      socket.to(normalizeChatRoom(chatId)).emit('user_typing', { chatId, userId: uid, userName: userName || uid });
    ...
    socket.on('message_seen', async ({ messageId, chatId }) => {
      ...
      } catch (e) { console.error('message_seen error:', e); }
    });
    ...
    socket.on('toggle_reaction', async ({ chatId, messageId, emoji }) => {
      ...
      try {
        const message = await prisma.message.findFirst({
          where: { id: messageId, chatId, chat: { OR: [{ isGlobal: true }, { participants: { some: { id: uid } } }] } },
          select: { id: true, chatId: true },
        });
        if (!message) return;
        const existing = await prisma.messageReaction.findFirst({ where: { messageId, userId: uid, emoji }, select: { id: true } });
        if (existing) {
          await prisma.messageReaction.delete({ where: { id: existing.id } });
        } else {
          await prisma.messageReaction.create({ data: { messageId, userId: uid, emoji } });
        }
        ...
        chatNs.to(normalizeChatRoom(chatId)).emit('message_reactions', { messageId, reactions: shapeReactionGroups(reactions) });
      } catch (e) { console.error('toggle_reaction error:', e); }
    });
    ...
    socket.on('disconnect', async () => {
      ...
      if (uid) {
        ...
        } catch (e) { console.error('disconnect presence error:', e); }
      }
    });
  });
}
